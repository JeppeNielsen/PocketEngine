...
void IOSBuilder::Build(const std::string &outputPath, const std::string &pocketEngineLibPath) {

    ProjectBuilder& p = *project;

    const std::string GameName = "Game";
    std::string gameDir = p.workingDirectory+"/"+GameName;
    ...
    std::string payloadPath = p.workingDirectory +"/Payload";
    ...
    FileReader::RunCommmand("cd "+p.workingDirectory+" && zip -r "+ outputIPAPath+" Payload");
    
}

void IOSBuilder::CreateXCodeProject(const std::string &outputFile) {
...
/* End XCConfigurationList section */
    ...
    auto headerPaths = project->GetEngineHeaders();
...
std::string IOSBuilder::GetXCodePlistTemplate() {
...
std::string IOSBuilder::GetSplashPngData() {
...
std::string IOSBuilder::GetMainFile() {
    std::string file = "";
