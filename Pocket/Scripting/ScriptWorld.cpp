...
\
    ...
    string compilerFlags = "-std=c++14 -stdlib=libc++ -DOSX "+pathToPocketEngineLib+" -o game -O3 -x objective-c++ -arch x86_64 -lobjc ";
...
void ScriptWorld::WriteExecutableMain(const std::string &path) {
    ...
    file<<"     "<<std::endl;
    file<<" GameObject* go = world.CreateObject();    "<<std::endl;
    file<<" go->AddComponent<Transform>()->Position = {0,0,10};    "<<std::endl;
    file<<" go->AddComponent<Camera>();    "<<std::endl;
    
    file<<" GameObject* object = world.CreateObject();    "<<std::endl;
    file<<" object->AddComponent<Transform>()->Position = {0,0,0};    "<<std::endl;
    file<<" object->AddComponent<Material>();    "<<std::endl;
    file<<" object->AddComponent<Mesh>()->GetMesh<Vertex>().AddCube(0,1);    "<<std::endl;
    file<<" object->AddComponent<Touchable>();    "<<std::endl;
    
    for(auto& component : components) {
        file<<" object->AddComponent<"<<component.first<<">();   "<<std::endl;
    }
    file<<"     "<<std::endl;
    file<<"    } "<<std::endl;
